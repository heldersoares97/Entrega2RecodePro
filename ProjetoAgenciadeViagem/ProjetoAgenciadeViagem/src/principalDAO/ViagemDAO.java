...
public class ViagemDAO {
    ...
    public void atualizarViagem(Viagem viagem) {
        String sql = "UPDATE viagem SET origem_Destino = ?, data_viagem = ? WHERE id_viagem = ?";
        try (PreparedStatement stmt = conexao.prepareStatement(sql)) {
            stmt.setString(1, viagem.getOrigemDestino());
            stmt.setTimestamp(2, new java.sql.Timestamp(viagem.getDataViagem().getTime()));
